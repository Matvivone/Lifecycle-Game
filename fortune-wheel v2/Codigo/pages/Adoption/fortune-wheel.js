...
function spinWheel() {
    ...
    outcomeIndex = (outcomeIndex + 1) % outcomes.length; // Cycle through the outcomes
    ...
    setTimeout(() => {
        ...
        const stopSpin = setInterval(() => {
            if (spinAngle % (2 * Math.PI) > finalAngle && spinAngle % (2 * Math.PI) < finalAngle + 0.2) {
                ...
                modalOption.textContent = `${segmentsName[selectedOutcome - 1]}!`;
